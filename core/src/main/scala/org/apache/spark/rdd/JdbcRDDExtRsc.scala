 ...
 */
class JdbcRDDExtRsc[T: ClassTag](
    sc: SparkContext,
    extRscName: String,
    sql: String,
    lowerBound: Long,
    upperBound: Long,
    numPartitions: Int,
    mapRow: (ResultSet) => T = JdbcRDDExtRsc.resultSetToObjectArray _)
  ...
  override def getPartitions: Array[Partition] = {
    ...
    (0 until numPartitions).map(i => {
      ...
      new JdbcPartitionExt(i, start, end)
    }).toArray
  }
...
////          conn.close()
//          rsc.putInstances(context.attemptId)
//        }
