...
import org.apache.spark.sql.hive.orc.{OrcTableScan, OrcRelation}
  ...
  object OrcOperations extends Strategy {
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      case logical.WriteToOrcFile(path, child) =>
        ...
        orc.InsertIntoOrcTable(relation, planLater(child), overwrite=true) :: Nil
      case logical.InsertIntoOrcTable(table: OrcRelation, partition, child, overwrite) =>
        orc.InsertIntoOrcTable(table, planLater(child), overwrite) :: Nil
