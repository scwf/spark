...
class ParquetQuerySuite extends QueryTest with FunSuiteLike with BeforeAndAfterAll {
  ...
  test("Compression options for writing to a Parquetfile") {
    ...
    var actualCodec = ParquetTypesConverter.readMetaData(new Path(path), Some(TestSQLContext.sparkContext.hadoopConfiguration)).get
    ...
    rdd.saveAsParquetFile(path)
    actualCodec = ParquetTypesConverter.readMetaData(new Path(path), Some(TestSQLContext.sparkContext.hadoopConfiguration)).get
      .getBlocks.flatMap(block => block.getColumns).map(column => column.getCodec.name()).distinct
    assert(actualCodec === TestSQLContext.parquetCompressionCodec.toUpperCase :: Nil)
  ...
  test("Writing metadata from scratch for table CREATE") {
    ...
    val metaData = ParquetTypesConverter.readMetaData(path, Some(ContextUtil.getConfiguration(job))).get
