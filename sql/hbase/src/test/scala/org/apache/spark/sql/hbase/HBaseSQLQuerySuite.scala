...
class HBaseSQLQuerySuite extends HBaseIntegrationTestBase {
  ...
  test("index into array") {
    checkAnswer(
      ...
      arrayData.map(d => (d.dt, d.dt(0), d.dt(0) + d.dt(1), d.dt(1))).collect().toSeq)
  }
  ...
  def sortTest() = {
    ...
    checkAnswer(
      ...
      arrayData.collect().sortBy(_.dt(0)).toSeq)

    checkAnswer(
      sql("SELECT * FROM arrayData ORDER BY dt[0] DESC"),
      arrayData.collect().sortBy(_.dt(0)).reverse.toSeq)
  ...
  test("SPARK-3813 CASE a WHEN b THEN c [WHEN d THEN e]* [ELSE f] END") {
    checkAnswer(
      sql("SELECT CASE k WHEN 1 THEN 1 ELSE 0 END FROM testData WHERE k = 1 group by k"), 1)
  }

  test("SPARK-3813 CASE WHEN a THEN b [WHEN c THEN d]* [ELSE e] END") {
    checkAnswer(
      sql("SELECT CASE WHEN k = 1 THEN 1 ELSE 2 END FROM testData WHERE k = 1 group by k"), 1)
  }
  ...
  test("Test to check we can use Long.MinValue") {
    checkAnswer(
      sql(s"SELECT ${Long.MinValue} FROM testData ORDER BY k LIMIT 1"), Long.MinValue
    )

    checkAnswer(
      sql(s"SELECT k FROM testData WHERE k > ${Long.MinValue}"), (1 to 100).map(Row(_)).toSeq
    )
  }
  ...
  test("Test to check we can apply sign to expression") {
    ...
    checkAnswer(
      sql("SELECT -k FROM testData WHERE k = 2"), -2
    )

    checkAnswer(
      sql("SELECT +k FROM testData WHERE k = 3"), 3
    )

    checkAnswer(
      sql("SELECT -(k + 1) FROM testData WHERE k = 1"), -2
    )

    checkAnswer(
      sql("SELECT - k + 1 FROM testData WHERE k = 10"), -9
    )

    checkAnswer(
      sql("SELECT +(k + 5) FROM testData WHERE k = 5"), 10
    )

    checkAnswer(
      sql("SELECT -MAX(k) FROM testData"), -100
    )
    ...
    checkAnswer(
      sql("SELECT + (-k) FROM testData WHERE k = 32"), -32
    )
    ...
    checkAnswer(
      sql("SELECT + - k FROM testData WHERE k = 33"), -33
    )
  }
  ...
  test("SPARK-3814 Support Bitwise & operator") {
    checkAnswer(sql("SELECT k&1 FROM testData WHERE k = 1 "), 1)
  }

  test("SPARK-3814 Support Bitwise | operator") {
    checkAnswer(sql("SELECT k|0 FROM testData WHERE k = 1 "), 1)
  }
  ...
  test("SPARK-3814 Support Bitwise ~ operator") {
    checkAnswer(sql("SELECT ~k FROM testData WHERE k = 1 "), -2)
  }
